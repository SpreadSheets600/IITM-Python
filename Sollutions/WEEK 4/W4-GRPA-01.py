...
int_iterable_sorted = sorted(
    int_iterable
)  # list: the int_iterable sorted in ascending order
int_iterable_sorted_desc = sorted(
    int_iterable, reverse=True
...
if isinstance(
    some_collection, (list, tuple)
):  # some collections are not indexable why?
    third_last_element = some_collection[
        -3
...
if isinstance(
    some_collection, (list, tuple, range)
):  # some collections are not slicable
...
is_some_value_in_some_collection = (
    some_value in some_collection
...
if isinstance(some_collection, (str, list, tuple)):  # some collections are not ordered
    is_some_value_in_even_indices = (
        some_value in some_collection[::2]
...
all_iterables = (
    list(some_iterable) + list(another_iterable) + list(yet_another_iterable)
